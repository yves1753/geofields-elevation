...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Contact Geofields Tanzania Limited in Dar es Salaam. Request a quotation, chat on WhatsApp, or reach our operations team by phone or email." },
    ],
    ...
    <Layout>
      ...
      <section className="section-y bg-background">
        <div className="container-x grid lg:grid-cols-[1.1fr_1fr] gap-16">
          <Reveal>
            <span className="eyebrow"><span className="h-[1px] w-10 bg-primary" /> Request a Quotation</span>
            <h2 className="mt-4 text-4xl md:text-5xl leading-tight">Tell us about your project.</h2>
            <p className="mt-4 text-muted-foreground">We'll match you with the right technical lead and reply within one business day.</p>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="mt-10 grid sm:grid-cols-2 gap-5">
              ...
              <Field label="Project Details" className="sm:col-span-2">
                <textarea rows={5} {...register("message", { required: "Required" })} className="input" />
              </Field>
              <div className="sm:col-span-2 flex items-center justify-between gap-4 flex-wrap">
                <button type="submit" className="btn-primary">Submit Enquiry <HiOutlineArrowRight /></button>
                {sent && <span className="text-sm text-green-700">Thank you — opening your mail client…</span>}
              </div>
            </form>
          </Reveal>
          ...
          <Reveal delay={0.15}>
            <div className="bg-[oklch(0.14_0.005_60)] text-white p-10 h-full">
              <span className="eyebrow-light"><span className="h-[1px] w-10 bg-primary-glow" /> Head Office</span>
              <h3 className="mt-4 text-white text-3xl">Dar es Salaam, Tanzania</h3>

              <div className="mt-10 space-y-6">
                <InfoRow icon={HiOutlineLocationMarker} title="Address">
                  Mwai Kibaki Road, Mbezi Beach<br />P.O Box 76387, Dar es Salaam, Tanzania
                </InfoRow>
                <InfoRow icon={HiOutlinePhone} title="Telephone">
                  <a href="tel:+255766775255" className="block hover:text-primary-glow">+255 766 775 255</a>
                  <a href="tel:+255755284141" className="block hover:text-primary-glow">+255 755 284 141</a>
                </InfoRow>
                <InfoRow icon={HiOutlineMail} title="Email">
                  <a href="mailto:info@geofields.co.tz" className="block hover:text-primary-glow">info@geofields.co.tz</a>
                  <a href="mailto:sales@geofields.co.tz" className="block hover:text-primary-glow">sales@geofields.co.tz</a>
                </InfoRow>
                <InfoRow icon={HiOutlineClock} title="Business Hours">
                  Mon – Fri · 08:00 – 17:00 EAT<br />Sat · 09:00 – 13:00
                </InfoRow>
              </div>

              <div className="mt-10 pt-8 border-t border-white/10 flex flex-wrap gap-3">
                <a href="https://wa.me/255766775255" target="_blank" rel="noreferrer" className="btn-primary">
                  <FaWhatsapp /> WhatsApp
                </a>
                <a href="tel:+255766775255" className="btn-ghost">Call Now</a>
              </div>
            </div>
          </Reveal>
        </div>
...
function Field({ label, error, className = "", children }: { label: string; error?: string; className?: string; children: React.ReactNode }) {
  return (
    <label className={`block ${className}`}>
      <span className="text-xs uppercase tracking-[0.2em] font-semibold text-foreground/70">{label}</span>
...
function InfoRow({ icon: Icon, title, children }: { icon: React.ComponentType<{ className?: string }>; title: string; children: React.ReactNode }) {
  return (
    <div className="flex gap-4">
      ...
      <div>
        <div className="text-xs uppercase tracking-[0.2em] text-white/50 font-semibold">{title}</div>

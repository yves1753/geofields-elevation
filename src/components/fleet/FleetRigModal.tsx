...
const fields: [keyof FleetRig, string][] = [
  ["rigType", "Rig Type"],
  ["maximumDrillDepth", "Maximum Drill Depth"],
  ["holeSize", "Hole Size"],
  ["drillingMethod", "Drilling Method"],
  ["enginePower", "Engine Power"],
  ["pullbackCapacity", "Pullback Capacity"],
  ["pulldownCapacity", "Pulldown Capacity"],
  ["rotationSpeed", "Rotation Speed"],
  ["drillAngle", "Drill Angle"],
          ...
          <div className="p-6 md:p-9">
            ...
            <dl className="mt-7 border border-border rounded-lg overflow-hidden grid md:grid-cols-2">
              {fields.map(([key, label]) => (
                <div
                  key={key}
                  className="p-4 border-b border-border md:odd:border-r last:border-b-0"
                >
            ...
            {rig.applications.length > 0 && (
              <div className="mt-7">
                <h3 className="text-lg">Operational applications</h3>
                <ul className="mt-3 grid sm:grid-cols-2 gap-2 text-sm text-muted-foreground">
                  {rig.applications.map((a) => (
                    <li key={a}>• {a}</li>
                  ))}
                </ul>
              </div>
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3">
              <a
                href="mailto:sales@geofields.co.tz?subject=Equipment%20information%20request"
